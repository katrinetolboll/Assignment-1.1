...
library(lmerTest)
# testing 
diagnosisTest = lmer(Diagnosis ~ CHI_MLU*tokens_CHI*types_CHI*verbalIQ + (1 + Age|ID), data = data, REML = FALSE)
summary(diagnosisTest)
# correct?? 
## Hypothesis 2 
mluHypothesis = lmer(CHI_MLU ~ Age + Diagnosis + (1 + Age|ID), data = data)
mluHypothesis
...
```{r}
## Hypothesis MOT MLU 
mluMotHypothesis = lmer(MOT_MLU ~ Age + Diagnosis + (1 + Age|ID), data = data)
...
```{r}
cltModel1 = lmer(CHI_MLU ~ Age + Diagnosis + Ethnicity + (1 + Age|ID), data = data)
summary(cltModel1)
cltModel2 = lmer(CHI_MLU ~ Age + Diagnosis + nonVerbalIQ + (1 + Age|ID), data = data)
summary(cltModel2)
cltModel3 = lmer(CHI_MLU ~ Age + Diagnosis + verbalIQ + (1 + Age|ID), data = data)
summary(cltModel3)
cltModel4 = lmer(CHI_MLU ~ Age + Diagnosis + Ethnicity + nonVerbalIQ + verbalIQ + (1 + Age|ID), data = data)
summary(cltModel4)
## run Anova 
anova(cltModel1, cltModel2, test = "Chisqr")
anova(cltModel1, cltModel2, cltModel3, cltModel4, test = "Chisqr")
## ???????? 
