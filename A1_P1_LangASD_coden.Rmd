...
```{r}
demo_train <- read.csv("~/Assignment-1.1/demo_train(1).csv")
LU_train <- read.csv("~/Assignment-1.1/LU_train.csv")
token_train <- read.csv("~/Assignment-1.1/token_train.csv")
...
names(demo_train)[names(demo_train)=="MullenRaw"]="nonVerbalIQ"
names(demo_train)[names(demo_train)=="ExpressiveLangRaw"]="verbalIQ"
...
data1=merge(LU_train,token_train,by=c("ID","Visit"))
  
data=merge(data1,demo_train,by=c("ID","Visit")) 
...
data$Gender = revalue(data$Gender, c("1"="F", "2"="M"))
...
write.csv(data,file="AutismData.csv")
...
#1.
Child_mlu=filter(data,CHI_MLU>2.7)
length(Child_mlu)
Child_mlu$ID
unique(Child_mlu$ID)
...
#2
Child_mlu2=filter(data,CHI_MLU<1.5,Visit==1)
unique(Child_mlu2$ID)
...
#Child ID 55 produced the most words on the 6th visi

#2
visit1 = subset(data[data$Visit == 1,])
arrangedvisit1 = arrange(visit1,(tokens_CHI))
head(arrangedvisit1)
#Child ID 57 produced the least words on the 1th visi
...
subs=select(data,ID,Visit,Diagnosis,CHI_MLU,tokens_CHI)
subs=filter(subs,Diagnosis=="ASD")

#2
subs2=select(data,ID,Visit,Diagnosis,CHI_MLU,tokens_CHI,tokens_CHI)
...
testdata = mutate(testdata, GrandChildAvg = mean(tokens_CHI))
...
TokensallVisits = testdata %>% 
              group_by(ID) %>%
              summarise(meanWords = mean(tokens_CHI+tokens_MOT))
#3
